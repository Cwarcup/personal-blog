...
draft: false
summary: Rules of functions, scope, errors, coercion, and more.
---
...
Day 2 of bootcamp! Today's lesson includes writing code incrementally, problem solving tips and basic git workflow (init -> add -> commit -> push). Additionally, we cover JavaScript specific concenpts such as debugging, pseudocode, and scope.

## Function Best Practices

1. Use precise verb/action based names
2. Use `camelCasedNames` for functions (and variables)
3. use two spaces to indent code
4. Functions should only do one thing

#### Function Best Practices - Rolling Dice

Create a function that accepts a single parameter from the command line (a number), and rolls that many six-sided dice.
...
- Globally-scoped variables are available to all functions, everywhere in that code file.

- Locally-scoped variables are only available within the function they are defined in.
...
const printVariables = function () {
  let myLocalVariable = 'local'
  console.log('-- Inside printVariables --')
  console.log('myLocalVariable is:', myLocalVariable)
  console.log('myGlobalVariable is:', myGlobalVariable)
}

printVariables()

console.log('-- Outside of printVariables now --')
...
const myFunction = function () {
  ...
  console.log('inside myFunction, variable is:', variable) // "local"
}

myFunction()

console.log('outside myFunction, variable is:', variable) // "global"
```
