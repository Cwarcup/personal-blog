...
draft: false
summary: Sharing functions with modules and unit testing with Mocha and Chai.
---
...
```js
const hello = require('./someModule')
```
...
```js
const hello = require('./someModule.js')
```
...
When we add `require('./someFile')`, the `.` means we are looking in the same directory as the file that is importing it. Doing `../` means we are looking in the parent directory.

You do not need to include the file extension when importing.
...
The `scripts` portion of `package.json` allows us to run commands using an **alias**, for instance:
...
[Mocha](https://mochajs.org/) provides us with the `describe()` and `it()` functions. These are required for each test we run.

[Chai](https://www.chaijs.com/) contains the assertion functions that we use to check our tests. It's a library that provides us with a bunch of useful assertion functions. Chai is designed to work with Mocha.
...
const nameInverter = function (name) {
  ...
  for (let word of honorifics) {
    if (name === word) {
      return ''
    }

    if (name.includes(word) && name.split(' ').length <= 2) {
      return name
    }
    if (name.includes(word) && name.split(' ').length > 2) {
...
```bash
vagrant [nameFormatter]> mocha


  nameInverter
    ✔ should return an empty string when passed an empty string
    ✔ should return a single name when passed a single name
    ✔ should return a single name when passed a single name with extra spaces
    ✔ should return a last-name, first-name when passed a first and last-name
    ✔ should return an empty string when passed a single honorific
    ✔ should return honorific first-name when passed honorific first-name
    ✔ should return a honorific last-name, first-name when passed honorific first-name last-name
