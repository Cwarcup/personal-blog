...
Annonymous function simply means that the function is not named (not declared), but is rather inline like so:
...
Recall that in Javascript, functions have access to..

1. variables in its own context (local variables)
2. variables in the lexical environment/parent fucntion (the scope of the function in which it was defined)
3. Global variables (variables defined outside of all functions)

The concept of closure is that a function has access to the variables in its lexical environment. Meaning, the function has access to the variables in the scope of the function in which it was defined.
