...
A lot of Node callbacks use an _error-first_ approach where the first argument to the callback function is an **error** (if any) or null, and the second argument is the **data**.

This can result in duplicated error handling logic in each callback in the chain.
...
Promises allow us to handle error in the _Promise chain_ with a single `.catch()` on the end of the chain.
...
Instead of using a `.catch()`, we could use the following:

```js
get('example.json').then(resolveFunction).then(undefined, rejectFunction)
```

This is the same as:

```js
get('example.json').then(resolveFunction).catch(rejectFunction)
```

This is because as soon as a promise is rejected, it will be passed to the next `.then()` in the chain. However, you should just use `.catch()` if you want to handle the error.
...
These methods accept an iterable as the argument. Usually this takes the form of an array.
...
We can use the `Promise` constructor to create a promise. It takers a callback function that takes two arguments: `resolve` and `reject`.
